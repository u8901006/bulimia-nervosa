...
1. 從提供的論文清單中，篩選出最具臨床實用性與新穎性的論文
2. 為每篇論文提供繁體中文摘要，包含重點發現與 PICO 分析
3. 評估每篇論文的臨床實用性（高/中/低）
4. 生成適合臨床專業人員閱讀的繁體中文報告

輸出格式要求：
- 語言：繁體中文（台灣用語）
- 專業但易讀
- 每篇論文須包含：中文標題、一句話總結、PICO分析、臨床實用性、相關標籤
- 最後提供今日 TOP 5（最重要/最影響臨床實用性的論文）
  ...
  if (start >= 0 && end > start) {
    ...
    for (let attempt = 0; attempt < 5; attempt++) {
      try {
        ...
        if (lastBrace > 0) sub = sub.slice(0, lastBrace) + "}";
        else break;
      }
    }
  }
...
async function callNvidiaAPI(apiKey, papersData) {
  ...
  const prompt = `以下是 ${dateStr} 從 PubMed 抓到的最新暴食症（Bulimia Nervosa）研究論文（共 ${paperCount} 篇）。

請進行以下分析，並以 JSON 格式回傳（不要用 markdown code block 包裹）：

{
  "date": "${dateStr}",
  "market_summary": "1-2句話總結今日暴食症研究的趨勢與亮點",
  "top_picks": [
    {
      "rank": 1,
      "title_zh": "中文標題",
      ...
      "journal": "期刊名",
      "summary": "一句話總結（繁體中文，指出核心發現與臨床實用性）",
      "pico": {
        "population": "研究對象",
        "intervention": "介入措施",
        "comparison": "對照組",
...
每篇 paper 的 tags 清單從以下選擇：心理治療、藥物治療、神經科學、營養學、運動醫學、流行病學、身體意象、社會文化、評估工具、基因學、併發症、兒少精神醫學、復原與預防、數位介入、共病。
...
<title>BN Research Daily &middot; 暴食症研究文獻日報 &middot; ${dateDisplay}</title>
<meta name="description" content="${dateDisplay} 暴食症研究文獻日報，由 AI 自動彙整 PubMed 最新論文"/>
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .link-card { display: flex; align-items: center; gap: 14px; padding: 16px 22px; background: var(--card-bg); border: 1px solid var(--line); border-radius: 20px; text-decoration: none; color: var(--text); transition: all 0.2s; box-shadow: 0 8px 30px rgba(61,36,15,0.04); margin-bottom: 10px; }
...
<div class="container">
  <header>
    ...
    <div class="header-text">
      <h1>BN Research Daily &middot; 暴食症研究文獻日報</h1>
  ...
  ${summary ? `<div class="summary-card"><h2>📋 今日文獻趨勢</h2><p class="summary-text">${esc(summary)}</p></div>` : ""}
...
async function main() {
  ...
  try {
    ...
    console.error(`[ERROR] Failed to load papers: ${e.message}`);
    process.exit(1);
  }
  ...
  if (!papersData.papers || papersData.papers.length === 0) {
    ...
    papersData = {
      ...
      _analysis: {
        date: dateStr,
        market_summary: "今日 PubMed 暫無新的暴食症研究文獻更新。請明天再查看。",
        top_picks: [],
        all_papers: [],
        keywords: [],
        topic_distribution: {},
